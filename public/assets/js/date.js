...
function Month(index) {
    ...
    const longNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September",
...
function updateDateField(e, fullDate, dateField) {
    ...
    // console.table(dateField);
    // console.table(dateField.name);
     setCookie(dateField.name, fullDate, 30);
...
setTimeout(() => {
    ...
    const start = date.getAttribute('value');
    if (start !== null) {
        date.value = start;
    }
    const dateend = document.getElementById('type1-deadline');
    const end = dateend.getAttribute('value');
    if (end !== null) {
        dateend.value = end;
    }
...
function drawTable(e, calendar) {
    ...
    monthText.textContent = `${month.shortName} ${year}`
    ...
    while (count <= monthDays) {
        ...
        for (i = 0; i < 7; i++) {
            ...
            if (count < 1) {
                ...
                let helpText = document.createElement(
                    "span"); // Originally intended to show the today's date. Removed.
                let cellText = document.createElement("span"); // The number inside each date cell
                ...
                if (!(cell.classList.contains("disabled") || cell.classList.contains("empty"))) {
                    ...
                    cell.addEventListener("click", function(e) {
                        ...
                        let fullDate = `${cellText.textContent} ${month.shortName} ${year} `
...
function editBtnListener(widget, minData, maxData) {
    ...
    if (maxData) {
        ...
        maxYear = thisYear + 1;
        maxMonth = new Month(thisMonth.index - 2);
    }
...
function toggleCalendar(e, widget) {
    ...
    const calendarPadding = Number(window.getComputedStyle(calendars[0]).getPropertyValue("padding-bottom")
        .replace(/px/, ''));
    const calendarMargin = Number(window.getComputedStyle(calendars[0]).getPropertyValue("margin-top").replace(
        /px/, ''));
...
calendarWidgets.forEach(widget => {
    ...
    if (widget.classList.contains("default-today")) {
        ...
        dateField.value = `${currDate} ${thisMonth.shortName} ${thisYear}`;
        dateField.readonly = true;
    }

    if (widget.classList.contains("default-today-return")) {
        dateField.readonly = false;
        dateField.value = `${currDate+1} ${thisMonth.shortName} ${thisYear}`;
        dateField.readonly = true;
    }
